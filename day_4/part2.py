...
for match in matches:
    cards[int(match[0])] = 1 if not cards.__contains__(int(match[0])) else cards[int(match[0])] + 1
    root = None
    number = ''
    numbers = match[1] + ' '
    index = 0
        ...
        if numbers[index].isdigit():
            ...
            if number != '':
                ...
                if node.value == int(number):
                    howmany += 1
                    for i in range(cards[int(match[0])]):
                        cards[int(match[0])+howmany] = 1 if not cards.__contains__(int(match[0])+howmany) else cards[int(match[0])+howmany] + 1
            number = ''
        index += 1
